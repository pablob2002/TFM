...
mpl.rcParams.update({'figure.dpi':150,'savefig.dpi':300,'font.size':13,'axes.labelsize':14,
    'axes.titlesize':14,'legend.fontsize':12,'xtick.labelsize':12,'ytick.labelsize':12,
...
axin.set_ylim(0,100); axin.set_xlabel(r"$\alpha$",fontsize=11,labelpad=1)
axin.set_ylabel("Percentile in pool (%)",fontsize=11,labelpad=1)
axin.yaxis.set_major_formatter(mticker.PercentFormatter())
axin.tick_params(labelsize=10); axin.set_title("ECDF",fontsize=12,pad=2)
for s in ['top','right']: axin.spines[s].set_visible(False)

fig.savefig(os.path.join(_FIGDIR, "coord_combined.png"),dpi=300,bbox_inches="tight")
fig.savefig(os.path.join(_FIGDIR, "coord_combined.pdf"),bbox_inches="tight")
